...
function parseMarkdown(
  ...
  return new Promise((resolve, reject) => {
    ...
    var body =
      FrontMatterEnd !== -1 ? markdown.substring(FrontMatterEnd + 3) : markdown;
    var header =
      FrontMatterEnd !== -1
        ? markdown.substring(
            FrontMatterStart + 3,
    ...
    var pushHeading = (root, heading, currentLevel, targetLevel) => {
      ...
      if (currentLevel === targetLevel) {
        ...
        pushHeading(
          root.childrens[root.childrens.length - 1],
          heading,
          currentLevel + 1,
          targetLevel
        );
      ...
      for (
        ...
        if (
          ...
          if (controlData.currentLevel === level) {
            ...
            console.log("Header control", JSON.stringify(col.heading));
          }
        }
        openNext += `<div data-Flowid="${flowId}-${index}" style="border:0px dashed grey ;border-top:0px dashed grey;padding:0px; ${style(
          index
        )};${controlStyle}  ">`;
      }
      console.log("Heading", flowId, level, text, headerControl);
      var header =
        level === 1
          ? ""
          : headerControl.hidden
          ? ""
          : `<h${level} className=${className} style="${headerStyle(
              level
            )};display:block;">${text}</h${level}>`;
    ...
    renderer.image = function(href, title, text) {
      ...
      switch (text ? text.toUpperCase() : "") {
        ...
        default:
          ...
          //   text = _.replace(text,"&quot;",`"` )
          url =
            href.substring(0, 4) === "http" || href.substring(0, 4) === "data"
              ? href
              : baseurl + href;
          if (text && startswith(text, "{")) {
            ...
            try {
              ...
              console.warn(
                "Markdown property parser error ",
                error,
                "source",
                text
              );
    ...
    renderer.link = function(href, title, text) {
      ...
      switch (tag ? tag.toUpperCase() : "") {
        case "JUMTOTAG":
          ...
          var search =
            questionMarkIndex > -1 ? href.substring(questionMarkIndex + 1) : "";
        ...
        case "CONTROL":
          ...
          json = unescape(json, /(\\)/gi, "");
          controlData = null;
          console.log("control data source", json);
          try {
            controlData = JSON.parse(json);
          } catch (error) {
            console.log("error parsing control data", error.message);
          }
        ...
        default:
          if (
            _.startsWith(href, "https://graph.microsoft.com/") ||
            _.startsWith(href, "https://graph.jumpto365.com")
          ) {
            var s = href.split("#");
          ...
          if (internalPath) {
            ...
            // console.log(internalPath,href,"==>",url)
          } else {
...
function convertToMarkdown(arrayBuffer) {
  return new Promise((resolve, reject) => {
    mammoth.convertToMarkdown({arrayBuffer}).then((value, messages) => {
      resolve(value);
    });
  });
}
