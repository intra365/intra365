...
export default class ToolbarPublishedVersionsDialog extends Component {
  state = { items: [] };
  componentDidMount() {
          ...
          <Shimmer  /></div>
            ...
            {this.state.items.map((item, index) => {
              return (
                ...
                >
                  {item.ToolbarKey} - {moment(item.Created).format("YYYY-MM-DD hh:mm:ss")}
                </div>
              );
            })}
          </div>
        ...
        <DialogFooter>
          {/* <PrimaryButton
            ...
            onClick={() => {
              ...
              this.props
                .action(this.state.selectedItem.ToolbarKey+'@'+this.state.selectedItem.VersionNumber, this.state)
                .then(result => {
                  this.setState({ onSaveAs: false });
                  if (!this.props.onSuccess) return false;
                  this.props.onSuccess(this.state.selectedItem.ToolbarKey+'@'+this.state.selectedItem.VersionNumber);
                })
                .catch(error => {
                  this.setState({
                    newTag: null,
